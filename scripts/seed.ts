...
/** Must match `.firebaserc` default and the app’s `EXPO_PUBLIC_FIREBASE_PROJECT_ID` when using emulators. */
const app = admin.initializeApp({ projectId: "my-app-5abfa" });
...
async function resetPriorSeed(): Promise<void> {
  ...
  await db
    ...
    .doc("discount_welcome_001")
    .delete()
    .catch(() => undefined);
  await db
    .collection("discounts")
    .doc("discount_addons_exp")
    .delete()
    .catch(() => undefined);
...
async function seed() {
  ...
  const staff1Id = await createUser(
    "anna.staff@test.com",
    "test1234",
    "Anna Kellenberger",
    "staff",
  ...
  const staff2Id = await createUser(
    "marc.staff@test.com",
    "test1234",
    "Marc Dubois",
    "staff",
  ...
  console.log(
    "  ✓ Discount: ADDONS15 — 15% off all add-ons (EXPIRED — for Scenario 5)",
  );
  ...
  console.log(
    "  ✓ Appointments: 4 created (confirmed, scheduled, completed, canceled)",
  );

  console.log("\nSyncing Auth custom claims (role, clinicId, plan, seats)...");
  await syncCustomClaimsForAllClinicMembers(db, CLINIC_ID, auth);
  console.log(
    "  ✓ Custom claims set for all users in clinic (force token refresh on next sign-in if needed)",
  );
